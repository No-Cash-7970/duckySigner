...
## THREAT-029: Sensitive or secret data written into logs

- **Actor:** Developer/Maintainer, desktop wallet, development tools
- **Purpose:** To gather data for debugging and troubleshooting
- **Target:** Secret data such as wallet passwords and Algorand account keys
- **Action:** Development tool logs secret data entered into or generated by the software
- **Result of the action:** The secret data is exposed, possibly written onto disk
- **Occurrence likelihood**: Medium
- **Impact:** High
- **Threat type:** Information disclosure
- **Potential mitigations:**
  1. Do not log secret data
  2. If some logging of secret data is needed for development, ensure that the logs are only for development. For example, use the "Information" level of logging, which should be disabled in production.
  3. If there is little or no control over how and when data is logged, encrypt the secret data before doing the action that gets logged

[Back to top ↑](#table-of-contents)

## THREAT-030: Database file containing secret data is too big to fit into memory

- **Actor:** Desktop wallet, user
- **Purpose:** None, the user has a lot of data to store into the wallet
