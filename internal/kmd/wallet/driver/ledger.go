...
// XXX: Modified from https://github.com/algorand/go-algorand/tree/c2d7047585f6109d866ebaf9fca0ee7490b16c6a/daemon/kmd/wallet/driver/ledger.go
...
var ledgerWalletSupportedTxs = []types.TxType{types.PaymentTx, types.KeyRegistrationTx}
...
func (lwd *LedgerWalletDriver) CreateWallet(name []byte, id []byte, pw []byte, mdk types.MasterDerivationKey) error {
...
func (lw *LedgerWallet) ExportMasterDerivationKey(pw []byte) (types.MasterDerivationKey, error) {
	return types.MasterDerivationKey{}, errNotSupported
}
...
func (lw *LedgerWallet) ListKeys() ([]types.Digest, error) {
	...
	var addr types.Digest
	copy(addr[:], reply)
	return []types.Digest{addr}, nil
}

// ImportKey implements the Wallet interface.
func (lw *LedgerWallet) ImportKey(sk ed25519.PrivateKey) (types.Digest, error) {
	return types.Digest{}, errNotSupported
}

// ExportKey implements the Wallet interface.
func (lw *LedgerWallet) ExportKey(pk types.Digest, pw []byte) (ed25519.PrivateKey, error) {
	return ed25519.PrivateKey{}, errNotSupported
}

// GenerateKey implements the Wallet interface.
func (lw *LedgerWallet) GenerateKey(displayMnemonic bool) (types.Digest, error) {
	return types.Digest{}, errNotSupported
}

// DeleteKey implements the Wallet interface.
func (lw *LedgerWallet) DeleteKey(pk types.Digest, pw []byte) error {
	return errNotSupported
}

// ImportMultisigAddr implements the Wallet interface.
func (lw *LedgerWallet) ImportMultisigAddr(version, threshold uint8, pks []ed25519.PublicKey) (types.Digest, error) {
	return types.Digest{}, errNotSupported
}

// LookupMultisigPreimage implements the Wallet interface.
func (lw *LedgerWallet) LookupMultisigPreimage(types.Digest) (version, threshold uint8, pks []ed25519.PublicKey, err error) {
	return 0, 0, nil, errNotSupported
}

// ListMultisigAddrs implements the Wallet interface.
func (lw *LedgerWallet) ListMultisigAddrs() (addrs []types.Digest, err error) {
	return nil, nil
}

// DeleteMultisigAddr implements the Wallet interface.
func (lw *LedgerWallet) DeleteMultisigAddr(addr types.Digest, pw []byte) error {
	return errNotSupported
}

// SignTransaction implements the Wallet interface.
func (lw *LedgerWallet) SignTransaction(tx types.Transaction, pk ed25519.PublicKey, pw []byte) ([]byte, error) {
	pks, err := lw.ListKeys()
	if err != nil {
		return nil, err
	}
	...
	if (!pk.Equal(ed25519.PublicKey{})) && !pk.Equal(ed25519.PublicKey(pks[0][:])) {
		// A specific key was requested; return an error if it's not the one on the device.
		return nil, errKeyNotFound
	}
	pk = ed25519.PublicKey(pks[0][:])
	...
	if types.Address(pk) != tx.Sender {
		stxn.AuthAddr = types.Address(pk)
	}

	return msgpack.Encode(&stxn), nil
}

// SignProgram implements the Wallet interface.
func (lw *LedgerWallet) SignProgram(data []byte, src types.Digest, pw []byte) ([]byte, error) {
...
func (lw *LedgerWallet) MultisigSignTransaction(tx types.Transaction, pk ed25519.PublicKey, partial types.MultisigSig, pw []byte, signer types.Digest) (types.MultisigSig, error) {
	isValidKey := false
	for i := 0; i < len(partial.Subsigs); i++ {
		subsig := &partial.Subsigs[i]
		if subsig.Key.Equal(pk) {
			isValidKey = true
			break
		}
	}
...
func (lw *LedgerWallet) MultisigSignProgram(data []byte, src types.Digest, pk ed25519.PublicKey, partial types.MultisigSig, pw []byte) (types.MultisigSig, error) {
	isValidKey := false
	for i := 0; i < len(partial.Subsigs); i++ {
		subsig := &partial.Subsigs[i]
		if subsig.Key.Equal(pk) {
...
func (lw *LedgerWallet) signTransactionHelper(tx types.Transaction) (sig types.Signature, err error) {
	lw.mu.Lock()
	defer lw.mu.Unlock()
...
func (lw *LedgerWallet) sendTransactionMsgpack(tx types.Transaction) (sig types.Signature, err error) {
	var reply []byte

	tosend := msgpack.Encode(&tx)
	p1 := ledgerP1first
	p2 := ledgerP2more
...
func (lw *LedgerWallet) sendTransactionOldStyle(tx types.Transaction) (sig types.Signature, err error) {
	var msg []byte
	...
	msg = append(msg, uint64le(uint64(tx.Fee))...)
	...
	case types.PaymentTx:
		...
		msg = append(msg, uint64le(uint64(tx.Amount))...)
...
func (lw *LedgerWallet) signProgramHelper(_ []byte) (sig types.Signature, err error) {
