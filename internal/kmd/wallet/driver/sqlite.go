...
// XXX: Modified from https://github.com/algorand/go-algorand/tree/c2d7047585f6109d866ebaf9fca0ee7490b16c6a/daemon/kmd/wallet/driver/sqlite.go
...
const (
	...
	sqliteWalletDBOptions       = "_pragma=secure_delete(1)&_txlock=exclusive"
...
var sqliteWalletSupportedTxs = []types.TxType{types.PaymentTx, types.KeyRegistrationTx}
...
func walletMetadataFromDBPath(dbPath string) (metadata wallet.Metadata, err error) {
	...
	db, err := sqlx.Connect("sqlite", dbConnectionURL(dbPath))
	if err != nil {
...
func (swd *SQLiteWalletDriver) CreateWallet(name []byte, id []byte, pw []byte, mdk types.MasterDerivationKey) error {
...
func (swd *SQLiteWalletDriver) RenameWallet(newName []byte, id []byte, pw []byte) error {
	...
	// Connect to the database
	db, err := sqlx.Connect("sqlite", dbConnectionURL(sqWallet.dbPath))
	if err != nil {
		return errDatabaseConnect
	}
	defer db.Close()
...
func (sw *SQLiteWallet) ExportMasterDerivationKey(pw []byte) (mdk types.MasterDerivationKey, err error) {
	// Check the password
...
func (sw *SQLiteWallet) ImportKey(rawSK ed25519.PrivateKey) (addr types.Digest, err error) {
...
func (sw *SQLiteWallet) ExportKey(addr types.Digest, pw []byte) (sk ed25519.PrivateKey, err error) {
	// Check the password
	err = sw.CheckPassword(pw)
	if err != nil {
		return
...
func (sw *SQLiteWallet) fetchSecretKey(addr types.Digest) (sk ed25519.PrivateKey, err error) {
	// Connect to the database
	db, err := sqlx.Connect("sqlite", dbConnectionURL(sw.dbPath))
	if err != nil {
		err = errDatabaseConnect
...
func (sw *SQLiteWallet) GenerateKey(displayMnemonic bool) (addr types.Digest, err error) {
	// Connect to the database
	db, err := sqlx.Connect("sqlite", dbConnectionURL(sw.dbPath))
	if err != nil {
		err = errDatabaseConnect
...
// its address
func (sw *SQLiteWallet) generateKeyTxLocked(tx *sqlx.Tx) (addr types.Digest, err error) {
...
func (sw *SQLiteWallet) DeleteKey(addr types.Digest, pw []byte) (err error) {
	// Check the password
	err = sw.CheckPassword(pw)
	if err != nil {
		return
...
func (sw *SQLiteWallet) LookupMultisigPreimage(addr types.Digest) (version, threshold uint8, pks []ed25519.PublicKey, err error) {
	// Connect to the database
	db, err := sqlx.Connect("sqlite", dbConnectionURL(sw.dbPath))
	if err != nil {
		err = errDatabaseConnect
...
func (sw *SQLiteWallet) DeleteMultisigAddr(addr types.Digest, pw []byte) (err error) {
	// Check the password
	err = sw.CheckPassword(pw)
	if err != nil {
		return
...
func (sw *SQLiteWallet) ListMultisigAddrs() (addrs []types.Digest, err error) {
	// Connect to the database
	db, err := sqlx.Connect("sqlite", dbConnectionURL(sw.dbPath))
	if err != nil {
		err = errDatabaseConnect
...
func (sw *SQLiteWallet) SignTransaction(tx types.Transaction, pk ed25519.PublicKey, pw []byte) (stx []byte, err error) {
	// Check the password
	err = sw.CheckPassword(pw)
	if err != nil {
		return
	...
	if (slices.Equal(pk, ed25519.PublicKey{})) {
		sk, err = sw.fetchSecretKey(types.Digest(tx.Sender))
	} else {
		sk, err = sw.fetchSecretKey(types.Digest(pk))
	}
	if err != nil {
		return
	}
...
func (sw *SQLiteWallet) SignProgram(data []byte, src types.Digest, pw []byte) (sprog []byte, err error) {
	// Check the password
	err = sw.CheckPassword(pw)
	if err != nil {
		return
	...
	// Fetch the required key
	sk, err := sw.fetchSecretKey(types.Digest(src))
	if err != nil {
		return
	}
...
// using the key
func (sw *SQLiteWallet) MultisigSignTransaction(tx types.Transaction, pk ed25519.PublicKey, partial types.MultisigSig, pw []byte, signer types.Digest) (sig types.MultisigSig, err error) {
	// Check the password
	err = sw.CheckPassword(pw)
	if err != nil {
		return
	...
	if partial.Version == 0 && partial.Threshold == 0 && len(partial.Subsigs) == 0 {
		...
		version, threshold, pks, err = sw.LookupMultisigPreimage(types.Digest(tx.Sender))
		if err != nil {
			return
		}
...
// partially signed multisig transaction signature of the passed transaction
// using the key
func (sw *SQLiteWallet) MultisigSignProgram(data []byte, src types.Digest, pk ed25519.PublicKey, partial types.MultisigSig, pw []byte) (sig types.MultisigSig, err error) {
	// Check the password
	err = sw.CheckPassword(pw)
	if err != nil {
		return
