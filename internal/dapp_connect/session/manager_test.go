...
var _ = FDescribe("DApp Connect Session Manager", func() {
	...
	Describe("NewManager()", func() {
		It("creates a new session manager with given configuration", func() {
			sessionManager := session.NewManager(ecdh.X25519(), &session.SessionConfig{"", "", "somewhere/dc", 42, 8})
			Expect(sessionManager.DataDir).To(Equal("somewhere/dc"))
		...
		It("creates a new session manager with default configuration when no configuration is given", func() {
			sessionManager := session.NewManager(ecdh.X25519(), nil)
			Expect(sessionManager.DataDir).To(Equal(session.DefaultDataDir))
	...
	Describe("StoreSession()", func() {
		...
		It("adds session to the database file if it already exists", func() {
			dataDirName := ".test_dc_store_session_exists"
			curve := ecdh.X25519()
			sessionManager := session.NewManager(curve, &session.SessionConfig{
				DataDir: dataDirName,
			})
			...
			sessionId2B64 := base64.StdEncoding.EncodeToString(sessionId2.Bytes())
			est2 := time.Now().Add(1 * time.Minute)
			exp2 := time.Now().Add(10 * time.Minute)
			testSession2 := session.New(sessionKey2, dappId2, exp2, est2, &dc.DappData{
				Name:        "My DApp 2",
				URL:         "https://example.com",
			...
			storedSessionRow := db.QueryRow(fmt.Sprintf(
				"PRAGMA add_parquet_key('key', '%s'); FROM read_parquet('%s', encryption_config = {footer_key: 'key'}) WHERE id = ?;",
				base64.StdEncoding.EncodeToString(fileEncryptKey.Bytes()),
				dataDirName+"/"+sessionManager.SessionsFile,
			), sessionId2B64)
			storedSessionRow.Scan(
			...
			Expect(storedSessionKey).To(Equal(base64.StdEncoding.EncodeToString(sessionKey2.Bytes())))
			Expect(storedExp).To(BeTemporally("~", exp2, time.Second))
			Expect(storedEst).To(BeTemporally("~", est2, time.Second))
			Expect(storedDappId).To(Equal(base64.StdEncoding.EncodeToString(dappId2.Bytes())))
			Expect(storedDappName).To(Equal(testSession2.DappData().Name))
